...
func lifecycleMarkerTestCases() []markerTestCase {
	return []markerTestCase{
		{
			name:      "missing file returns 0",
			setup:     func(t *testing.T, dir, marker string) {},
			wantCount: 0,
		},
		{name: "one line returns 1", setup: func(t *testing.T, dir, marker string) {
			requireWriteFile(t, filepath.Join(dir, marker), "attach\n")
		}, wantCount: 1},
		{name: "two lines returns 2", setup: func(t *testing.T, dir, marker string) {
			requireWriteFile(t, filepath.Join(dir, marker), "attach\nattach\n")
		}, wantCount: 2},
		{
		...
		{name: "filesystem error propagated", setup: func(t *testing.T, dir, marker string) {
...
func TestLifecycleMarkerCount(t *testing.T) {
	for _, tt := range lifecycleMarkerTestCases() {
