...
so GitHub signs it (committer: GitHub, verified). No Python, no GITHUB_TOKEN.
  ...
  - git add <changed workflow file(s)>
  ...
  - Pre-PR check (CRITICAL): confirm the branch has exactly ONE commit ahead of
    ...
    redo cleanly:
      ...
      # re-apply your edit, then: git add <changed workflow file(s)>
      task github:app:sign-commit -- -m "<conventional-commit subject, 50 chars max>" "<body>"
...
STEP 7 — Open the PR as the app (no GITHUB_TOKEN):
  ...
  - PR_JSON=$(curl -s -X POST -H "Authorization: bearer $TOKEN" -H "Accept: application/vnd.github+json" https://api.github.com/repos/devsy-org/devsy/pulls -d "{\"title\":\"<subject>\",\"head\":\"ci-optimizer/<slug>\",\"base\":\"main\",\"draft\":true,\"body\":${PR_BODY}}")
