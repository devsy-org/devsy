...
STEP 5 — Open the PR as the app (no GITHUB_TOKEN):
  ...
  - PR_JSON=$(curl -s -X POST -H "Authorization: bearer $TOKEN" -H "Accept: application/vnd.github+json" https://api.github.com/repos/devsy-org/devsy/pulls -d "{\"title\":\"test(e2e): <short description>\",\"head\":\"integration-test/<slug>\",\"base\":\"main\",\"draft\":true,\"body\":${PR_BODY}}")
