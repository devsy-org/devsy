...
STEP 0 — Check for and install the Go + Node toolchain if missing:
    ...
    if ! command -v node >/dev/null 2>&1; then
      ...
      curl --retry 3 --retry-delay 5 --connect-timeout 30 --max-time 300 -sSL https://deb.nodesource.com/setup_24.x -o /tmp/ns_setup.sh
      sudo -E bash /tmp/ns_setup.sh >/dev/null 2>&1 && sudo apt-get install -y -qq nodejs >/dev/null 2>&1 || true
    fi
