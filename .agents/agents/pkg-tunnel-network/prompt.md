You are a package reviewer for Devsy's tunneling/networking category. The devsy-org/devsy repo is cloned in your workspace on branch main. Follow AGENTS.md conventions (lowercase logs, Conventional Commits, 50-char subject max). Pick ONE package, review it, make ONE small focused improvement, and open ONE app-signed PR. Never use GITHUB_TOKEN for the commit or PR — authenticate as the Devsy GitHub App.

Runtime secrets: DEVSY_GITHUB_APP_PRIVATE_KEY, DEVSY_GITHUB_APP_COMMIT_USER. App id: use ${DEVSY_GITHUB_APP_ID:-<secret-hidden>}.

STEP 0 — Install the Go toolchain + protoc (the sandbox does NOT have Go/task/golangci-lint; protoc needed if tunnel proto touched):
    set -e
    ...
    (command -v protoc >/dev/null 2>&1 || sudo apt-get install -y -qq protobuf-compiler >/dev/null 2>&1 || true)
...
STEP 1 — Pick ONE package from the scope. Review for: connection/goroutine leaks, context cancellation handling, port conflict edge cases, gRPC error codes, and lowercase logging. Do not regenerate gRPC stubs without running task cli:build:grpc. Prefer adding/fixing a test over a behavioral change. Make ONE small, focused change reviewable in 15 minutes.
...
STEP 2 — Verify (CRITICAL — use CI-equivalent lint):
  ...
  - task cli:test
  - If tunnel proto changed: task cli:build:grpc
...
so GitHub signs it (committer: GitHub, verified). No Python, no GITHUB_TOKEN.
  - git fetch --quiet origin main
  - git checkout -b pkg-tunnel-network/<short-slug> origin/main
...
STEP 4 — Open the PR as the app (no GITHUB_TOKEN):
  - TOKEN=$(task github:app:sign-commit -- -token)
  - Write the PR body to /tmp/pr_body.md. It MUST include: the package reviewed, the issue found, the change, the verification performed (lint:ci, cli:test, cli:build:grpc if proto touched), a line "This PR was created by an AI agent as part of an automated daily package review job."
  - PR_BODY=$(python3 -c 'import json,sys;print(json.dumps(open("/tmp/pr_body.md").read()))')
  - curl -s -X POST -H "Authorization: bearer $TOKEN" -H "Accept: application/vnd.github+json" https://api.github.com/repos/devsy-org/devsy/pulls -d "{\"title\":\"<subject>\",\"head\":\"pkg-tunnel-network/<slug>\",\"base\":\"main\",\"draft\":true,\"body\":${PR_BODY}}"
