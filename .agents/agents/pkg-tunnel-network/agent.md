...
STEP 0 — Check for and install the Go toolchain + protoc if missing:
    set -e
...
STEP 4 — Open the PR as the app (no GITHUB_TOKEN):
  ...
  - PR_JSON=$(curl -s -X POST -H "Authorization: bearer $TOKEN" -H "Accept: application/vnd.github+json" https://api.github.com/repos/devsy-org/devsy/pulls -d "{\"title\":\"<subject>\",\"head\":\"pkg-tunnel-network/<slug>\",\"base\":\"main\",\"draft\":true,\"body\":${PR_BODY}}")
