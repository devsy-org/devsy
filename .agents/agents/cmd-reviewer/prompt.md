...
STEP 5 — Open the PR as the app (no GITHUB_TOKEN):
  - TOKEN=$(task github:app:sign-commit -- -token)
  - Write the PR body to /tmp/pr_body.md. It MUST include: the cmd/ subdirectory reviewed, the issue found, the minimal change, the verification performed (task cli:lint:ci result, task cli:test result), a line "This PR was created by an AI agent (OpenHands) as part of an automated daily CLI review job.", and "Commit signature: Verified (GitHub-signed via the Devsy GitHub App createCommitOnBranch mutation)."
