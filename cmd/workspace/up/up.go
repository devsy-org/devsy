...
type UpCmd struct {
	...
	*flags.GlobalFlags
	...
	// See cmd.runDetached.
	Detach bool
	// Set only on the detached child re-exec (--task-id).
	taskID string
...
func (cmd *UpCmd) Run(
	...
	if t != nil {
		...
		// The submitting process's guessed label may not match the
		// resolved ID; client.Status() looks tasks up by this label.
