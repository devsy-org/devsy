...
func NewLogsCmd(globalFlags *flags.GlobalFlags) *cobra.Command {
	...
	c := &cobra.Command{Use: "logs [name]", Short: "Show Devsy machine diagnostic events", RunE: func(c *cobra.Command, args []string) error { return cmd.Run(c.Context(), args) }}
	cliflags.Add(c, cliflags.String(&cmd.After, "after", "", "An opaque diagnostics cursor"), cliflags.Int(&cmd.Limit, "limit", machinediagnostics.DefaultReadEvents, "Maximum diagnostic events"))
	c.Flags().BoolVarP(&cmd.Follow, "follow", "f", false, "Poll for new diagnostic events until interrupted")
	return c
}
	...
	for {
		...
		if mode == output.ModeJSON {
			...
			if result.Source.Availability != string(machinediagnostics.AvailabilityAvailable) {
				_, _ = fmt.Fprintf(os.Stderr, "Diagnostics: %s %s\n", result.Source.Availability, result.Source.Message)
			}
			if result.Cursor.State == machinediagnostics.CursorGap || result.Cursor.State == machinediagnostics.CursorReset {
				_, _ = fmt.Fprintf(os.Stderr, "Diagnostic history: %s (%s)\n", result.Cursor.State, result.Cursor.Reason)
			}
