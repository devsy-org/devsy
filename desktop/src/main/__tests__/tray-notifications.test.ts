...
describe("updateNotifies", () => {
  ...
  it("notifies once per version when enabled", () => {
    const first = updateNotifies(downloaded, "failures", undefined)
    expect(first.notifies).toBe(true)
    expect(first.version).toBe("1.1.0")
    expect(updateNotifies(downloaded, "failures", "1.1.0").notifies).toBe(false)
  })
...
describe("TrayNotifier", () => {
  ...
  it("deduplicates repeated snapshots of the same command", () => {
    const { notifier, sent } = makeNotifier(() => "failures", () => false)
