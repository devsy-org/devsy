...
describe("buildTrayMenuTemplate", () => {
  ...
  function model(
    partial: Partial<Parameters<typeof buildTrayMenuTemplate>[0]>,
  ) {
    return {
      workspaces: [],
  ...
  it("counts actually running workspaces from normalized statuses", () => {
    expect(
      countRunningWorkspaces([
  ...
  it("does not add an overflow link at or under the limit", () => {
    ...
    expect(items.some((item) => item.label?.includes("View All"))).toBe(false);
  });
  ...
  it("offers View Logs for failed, busy, and unknown rows instead of lifecycle actions", () => {
    ...
    expect(row("failed-ws").some((item) => item.label === "View Logs")).toBe(
      true,
    );
    expect(
      row("failed-ws").some((item) => item.label === "Stop Workspace"),
    ).toBe(false);
    expect(row("busy-ws").some((item) => item.label === "View Logs")).toBe(
      true,
    );
    expect(
  ...
  it("mirrors the startup toggles as native checkboxes", () => {
    ...
    const runAtStartup = submenu.find(
      (item) => item.label === "Run at Startup",
    );
...
describe("trayWorkspaceState", () => {
  it("maps statuses and jobs to row states", async () => {
    ...
    expect(trayWorkspaceState({ id: "a", status: '{"state":"stopped"}' })).toBe(
      "stopped",
    );
