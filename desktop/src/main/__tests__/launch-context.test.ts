...
describe("shouldSuppressInitialWindow", () => {
  it("suppresses only an automatic launch with both toggles on and a tray host", () => {
    expect(shouldSuppressInitialWindow(base, true, true)).toBe(true)
  })

  it("never suppresses an explicit launch", () => {
    expect(shouldSuppressInitialWindow(base, false, true)).toBe(false)
  })

  it("never suppresses without a tray host (GNOME without an extension)", () => {
    expect(shouldSuppressInitialWindow(base, true, false)).toBe(false)
  })
