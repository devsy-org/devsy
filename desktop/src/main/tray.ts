...
export function countRunningWorkspaces(
...
export function buildTrayMenuTemplate(
  ...
  const running = countRunningWorkspaces(model.workspaces, jobs)
  ...
  const workspaceItems: Electron.MenuItemConstructorOptions[] = shown.map(
    (workspace) => {
      ...
      const pending =
        ...
        model.pendingStarts.has(workspace.id) ||
        state === "busy"
...
export class AppTray {
  ...
  rebuildMenu(): void {
    ...
    const running = countRunningWorkspaces(
      this.deps.state.workspaceList(),
      jobs,
    )
  ...
  // Repeat clicks deduplicate through the pending sets and through
  // WorkspaceJobs itself, which rejects a second in-progress operation.
