...
export class ProClient implements TDebuggable {
  ...
  public watchWorkspacesProxy(
    ...
    cmd.stream(
      (event) => {
        if (event.type === "data") {
          const rawInstances =
            event.data as unknown as readonly ManagementV1DevsyWorkspaceInstance[]
  ...
  ): Promise<Result<ManagementV1DevsyWorkspaceInstance>> {
  ...
  private async read(): Promise<unknown> {
    ...
    try {
      ...
      lines.forEach((line) => {
        if (line.trim()) {
          try {
            const rawInstances: readonly ManagementV1DevsyWorkspaceInstance[] = JSON.parse(line)
            const workspaceInstances = rawInstances.map(
              (instance) => new ProWorkspaceInstance(instance)
            )
