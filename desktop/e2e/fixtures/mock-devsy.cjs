...
function defaultState() {
  return {
    ...
    // Background tasks submitted via `workspace up --detach`, keyed by id.
    tasks: {},
  }
}
...
// Older persisted state files predate the task store.
...
// Adds a completed workspace to state, mirroring what a real `up` produces.
...
function handleUp(args) {
  ...
  // `--detach` submits a background task instead of running to completion;
  // the caller polls it via `workspace task logs <id> --follow`.
...
function handleTaskLogs(args) {
  ...
  // Single-line NDJSON result envelope, matching pkg/devcontainer/config/envelope.go.
  out(
    JSON.stringify({
      kind: "result",
      outcome: "success",
