...
### Structured CLI errors {#structured-cli-errors}
...
Earlier versions of Devsy could race when bootstrapping the workspace metadata file used by the VS Code Browser tunnel, causing the first browser session to fail immediately after `devsy up --ide openvscode`. This has been fixed — if you previously hit this, simply re-run `devsy up` on the affected workspace. When Devsy exits because the workspace could not be located, it returns exit code `75`, which parent processes (including Devsy Desktop) treat as a transient signal and retry automatically.
