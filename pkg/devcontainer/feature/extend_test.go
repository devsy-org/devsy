...
func (suite *ExtendTestSuite) TestGenerateContainerEnvs_SubstitutesContainerEnvVars() {
	feature := &config.FeatureSet{
		Config: &config.FeatureConfig{
	...
	suite.Contains(result, "ENV PATH=/usr/local/share/nvm/current/bin:${PATH}")
	suite.Contains(result, "ENV NVM_DIR=/usr/local/share/nvm")
	suite.NotContains(result, "${containerEnv:PATH}")
}

func (suite *ExtendTestSuite) TestGenerateContainerEnvs_Empty() {
	feature := &config.FeatureSet{
		Config: &config.FeatureConfig{
			ContainerEnv: map[string]string{},
		},
...
func (suite *ExtendTestSuite) TestGenerateContainerEnvs_MultipleContainerEnvVars() {
	feature := &config.FeatureSet{
		Config: &config.FeatureConfig{
			ContainerEnv: map[string]string{
	...
	result := generateContainerEnvs(feature)
	suite.Contains(result, "ENV PATH=${HOME}/bin:${PATH}")
	suite.NotContains(result, "containerEnv")
}

func (suite *ExtendTestSuite) TestGenerateContainerEnvs_NoVarRefs() {
	feature := &config.FeatureSet{
		Config: &config.FeatureConfig{
			ContainerEnv: map[string]string{
				"MY_VAR": "static_value",
	...
	result := generateContainerEnvs(feature)
	suite.Contains(result, "ENV MY_VAR=static_value")
	suite.Contains(result, "ENV OTHER=/some/path")
}
