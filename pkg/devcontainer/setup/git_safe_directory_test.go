...
func (s *GitSafeDirectoryTestSuite) TestSetupResetPrecedingWildcardAllowsAdd() {
	if _, err := exec.LookPath("git"); err != nil {
		s.T().Skip("git not found; skipping safe.directory system config tests")
	}
	...
	preseedWildcard := exec.Command("git", "config", "--system", "--add", "safe.directory", "*")
	preseedWildcard.Env = append(os.Environ(), "GIT_CONFIG_SYSTEM="+configFile)
	out, err := preseedWildcard.CombinedOutput()
	s.Require().NoError(err, string(out))

	preseedReset := exec.Command("git", "config", "--system", "--add", "safe.directory", "")
	preseedReset.Env = append(os.Environ(), "GIT_CONFIG_SYSTEM="+configFile)
	out, err = preseedReset.CombinedOutput()
	s.Require().NoError(err, string(out))
...
func (s *GitSafeDirectoryTestSuite) TestSetupResetPrecedingCandidateReadds() {
	if _, err := exec.LookPath("git"); err != nil {
		s.T().Skip("git not found; skipping safe.directory system config tests")
	}
	...
	//nolint:gosec // G204: repoDir is an isolated test directory
	preseedCandidate := exec.Command(
		"git",
		"config",
		"--system",
		"--add",
	...
	preseedCandidate.Env = append(os.Environ(), "GIT_CONFIG_SYSTEM="+configFile)
	out, err := preseedCandidate.CombinedOutput()
	s.Require().NoError(err, string(out))
	...
	entries, err := getGitSafeDirectories(ctx)
	s.Require().NoError(err)
	s.Equal([]string{repoDir, "", repoDir}, entries)
}
