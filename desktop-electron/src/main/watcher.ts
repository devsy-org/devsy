...
export class Watcher {
  ...
  private async pollWorkspaces(): Promise<void> {
    try {
      const workspaces = await this.deps.cli.run<unknown[]>(["list", "--skip-pro"])
      const changed = this.deps.state.updateWorkspaces(workspaces as any[])
      if (changed) {
        this.send("workspaces-changed", { workspaces: this.deps.state.workspaceList() })
      }
    } catch {
  ...
  private async pollMachines(): Promise<void> {
    try {
      ...
      if (changed) {
        this.send("machines-changed", { machines: this.deps.state.machineList() })
      }
    } catch {
      // Silently ignore poll failures
    }
  ...
  private async pollContexts(): Promise<void> {
    try {
      const entries = await this.deps.cli.run<ContextEntry[]>(["context", "list"])
      ...
      if (changed) {
        const { contexts: ctxList, activeContext } = this.deps.state.contextList()
