...
Instead of building from a devcontainer each time, you can also create a workspace from a previously saved [workspace snapshot](../developing-in-workspaces/workspace-snapshots.mdx) with `devsy up --from-snapshot`, which restores the container filesystem and volumes directly and skips the build step entirely.
