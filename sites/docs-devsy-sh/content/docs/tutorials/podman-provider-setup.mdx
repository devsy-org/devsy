...
If your WSL distro does not have systemd available, start the API service manually instead (run it in the background or under a process manager so it survives after you close the terminal):
...
If it does not exist or is inactive, start it with `systemctl --user enable --now podman.socket`. On systemd-less environments (some WSL distros), run `podman system service --time=0 unix://$XDG_RUNTIME_DIR/podman/podman.sock &` instead.
...
`podman compose` is not a built-in implementation — Podman delegates to an externally installed Compose provider, either the `docker-compose` binary/plugin or the `podman-compose` Python package. Install one before relying on `podman compose`:
