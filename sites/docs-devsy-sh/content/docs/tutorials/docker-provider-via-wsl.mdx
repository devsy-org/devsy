...
### 5. Install and enable OpenSSH server in WSL for secure remote access

Rather than exposing the Docker daemon on an unauthenticated TCP port, the Windows Docker client will reach the Docker daemon in WSL securely over SSH. Install and start an SSH server inside your WSL distro:
...
WSL2 automatically forwards `localhost` ports from Windows into the WSL VM for listening sockets, so the SSH server will be reachable from Windows at `localhost` once it is running.

To avoid a password prompt every time the Docker CLI connects, set up SSH key-based authentication: generate a key pair on Windows with `ssh-keygen`, then append the resulting public key to `~/.ssh/authorized_keys` inside WSL.
...
Create a Docker context that connects to the WSL Docker daemon over SSH. Replace `<wsl-username>` with the username you set up in Step 2:
...
The first connection will prompt you to accept the SSH host key. If you did not set up key-based authentication in Step 5, you will also be prompted for the WSL user's password.

## Integrate Devsy with Docker in WSL

### 8. Setup Devsy Docker provider to use Docker installed in WSL

Set `Docker Host` as `ssh://<wsl-username>@localhost` (replace `<wsl-username>` with your WSL username) and `Docker Path` as `docker`.
