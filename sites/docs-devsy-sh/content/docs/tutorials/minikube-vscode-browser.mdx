...
A `hostPath` volume must exist inside the Minikube node itself (the VM or container that runs Kubernetes), not on your tutorial host machine. Create the directory inside the node using `minikube ssh`:

```sh
minikube ssh -- sudo mkdir -p /mnt/data/devsy
```
