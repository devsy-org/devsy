...
back to the local user, such as `sudo chown -R $(id -un):$(id -gn) .`. The reason this is neccesary is by default, when a new user is created by the container runtime,
...
`podman compose` is not a built-in implementation — Podman looks for an externally installed Compose provider (`docker-compose` or `podman-compose`) and delegates to it. If your `devcontainer.json` or workspace scripts call `docker-compose` directly, they will fail once `docker-compose` itself is not installed. Install a provider first, for example:

```bash
# Option 1: podman-compose (Python-based)
sudo apt-get install -y podman-compose

# Option 2: docker-compose (standalone binary/plugin)
