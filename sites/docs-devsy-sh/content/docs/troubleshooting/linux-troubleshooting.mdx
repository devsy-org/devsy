...
If `devsy workspace up` exits with the structured error code `DOCKER_PERMISSION_DENIED`, your user is not a member of the `docker` group. Add yourself and re-login:
...
See the [Structured CLI errors](./troubleshooting.mdx) reference for the full list of codes.
