...
This walkthrough targets an x86-64 host and guest image. Download the official ISO for [Ubuntu 22.04](https://old-releases.ubuntu.com/releases/22.04/) (`ubuntu-22.04.x-desktop-amd64.iso`). An arm64 Ubuntu 22.04 image is also published on that page, but VirtualBox's arm64 support and this walkthrough's steps are not validated against it.
